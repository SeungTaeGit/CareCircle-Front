...
export default function Hero() {
  ...
  // 일본어일 때는 'break-all'(글자 단위 줄바꿈)을 적용하고,
  // 한국어일 때는 기존의 'break-keep'(단어 단위 유지)을 적용합니다.
  const breakStyle = lang === 'ja' ? 'break-all' : 'break-keep';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-teal-100 text-teal-700 font-semibold mb-6">
              ...
              {t('2026 글로벌 피우다 프로젝트 선정작', '2026 グローバルピウダプロジェクト選定作')}
            </div>

            {/* 💡 일본어 줄바꿈 해결을 위해 동적 클래스(breakStyle) 적용 및 w-full 추가 */}
            <h1 className={`text-4xl sm:text-5xl md:text-6xl font-bold text-slate-900 leading-tight mb-6 w-full ${breakStyle}`}>
              {t('어르신을 돌봄의 대상에서', '高齢者をケアの対象から')}<br/>
              <span className="text-teal-600">{t('사회참여의 주체', '社会参加の主体')}</span>{t('로.', 'へ。')}
            </h1>

            {/* 💡 설명 부분도 동일하게 적용 */}
            <p className={`text-lg sm:text-xl text-slate-600 mb-8 leading-relaxed w-full ${breakStyle}`}>
              {t('음성 하나로 기록되는 일상, 한국과 일본을 잇는 시니어 취미 교류.', '音声一つで記録される日常、韓国と日本を繋ぐシニア趣味交流。')}<br/>
              {t('AI 기술로 따뜻한 소통の 다리를 놓습니다.', 'AI技術で温かいコミュニケーションの架け橋を築きます。')}
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button className="bg-white text-slate-900 border border-slate-200 px-8 py-4 rounded-xl font-bold text-lg hover:bg-slate-50 transition-colors shadow-sm flex items-center justify-center gap-2">
                <Play className="w-5 h-5 fill-slate-900" /> {t('소개 영상 보기', '紹介動画を見る')}
              </button>
            </div>
          </div>
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-2xl h-[400px] md:h-[500px]">
            ...
            <div className="absolute bottom-6 left-6 right-6 z-20 text-white">
              <div className="bg-white/20 backdrop-blur-md px-4 py-2 rounded-lg inline-block mb-3 border border-white/30 text-sm font-medium">
                🌱 {t('함께정원 레벨 4 달성!', 'ガーデンレベル4達成！')}
              </div>
              <p className={`font-bold text-xl md:text-2xl leading-snug w-full ${breakStyle}`}>
                "{t('오늘 부침개 부치는 소리를 녹음해서 일본 친구에게 보냈단다.', '今日、チヂミを焼く音を録音して日本の友達に送ったよ。')}"
              </p>
            </div>
          </div>
        </div>
